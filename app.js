...
const apiKey= '235ca0f68d36e0f6d87848ff817dbb26'
;
...
button.addEventListener('click', function(){
   ...
   fetch(`https://api.openweathermap.org/data/2.5/weather?q=${input.value}&units=metric&appid=${apiKey}`)
    ...
    window.wind.innerHTML=`<span>wind: </span> ${wind}<span> m/s</sup>`;

    window.humidity.innerHTML=`<span>Humidity: </span> ${Humidity}<span> %</sup>` ;
    
    window.pressure.innerHTML=`<span>Pressure: </span> ${Pressure}<span> hPa</sup>`;
